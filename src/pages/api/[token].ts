...
async function verifyEmailHandler(
    ...
    try {
        const result = jwt.verify(token as string, process.env.VERIFICATION_EMAIL_SECRET as string);
        ...
        if (user.emailVerified != null) {
            throw res.status(400).json({ message: "Este correo ya fue verificado previamente, ya puedes hacer login" });
        }
