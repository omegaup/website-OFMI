  ...
  return (
    ...
    >
      <Options
        ...
        label={
          <legend>
            Certifico que lei y entendi el{" "}
            <Link href="/reglamento">reglamento</Link> y la{" "}
            <Link href="/convocatoria">convocatoria</Link>, y que cumplo con los
            requisitos de eligibilidad, incluyendo mi certificacion como mujer o
            persona no binaria.
          </legend>
        }
        options={[
      ...
      />
      <AlphaText name="nombres" label="Nombre(s)" />
      <AlphaText name="apellidos" label="Apellido(s)" />
      <Options
        type="checkbox"
        name="pronombres"
        label="Pronombre"
        options={["ella", "elle", "el"]}
      />
      <DataList
        name="pais"
        label="Pais"
      ...
      <DataList
        ...
        label="Estado"
        validation={
          data.pais === "México" ? "strict" : /^[a-zA-ZÁÉÍÓÚáéíóú ]+$/g
        }
        options={data.pais === "México" && Estados}
      />
      <Birthday name="cumpleanos" label="Cumpleaños" ageRange={[18, 20]} />
      <Options
        type="radio"
        name="escolaridad"
        label="Escolaridad"
      ...
      />
      <AlphaNumText name="escuela" label="Escuela" />
      {data.pais === "México" &&
        errors.estado === 0 &&
        errors.cumpleanos === 0 && (
          <CURP
