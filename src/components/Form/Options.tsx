...
function Options({ type, name, label, options, value, error, isRequired = true }: IOptions) {
    if (!value || !error) {
        return false;
    };
    ...
    return (
        <fieldset>
                ...
                {options.map((option) => {
                    ...
                    return (
                        <li key={v}>
                            ...
                            />
                            <label htmlFor={`${name}-${v}`}>{isString ? option : option.label}</label>
                        </li>
                    );
                })}
            </ul>
            ...
            {isRequired && error.state === false && !value.state.length && (
                <em>{`Debes seleccionar ${type === 'checkbox' ? 'almenos una' : 'una'} opción del campo ${name}`}</em>
            )}
        </fieldset>
    );
};
